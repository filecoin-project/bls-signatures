...
use bls12_381::{
    ...
    Bls12, G1Affine, G2Affine, G1Projective, Gt, MillerLoopResult,
};
use pairing_lib::MultiMillerLoop;

#[cfg(feature = "blst")]
use blstrs::{Bls12, G1Affine, G2Affine, G1Projective, Gt, MillerLoopResult};
#[cfg(feature = "blst")]
...
impl From<Signature> for G1Projective {
...
impl From<Signature> for G1Affine {
    fn from(val: Signature) -> Self {
        val.0
    }
}
...
#[cfg(feature = "pairing")]
pub fn hash(msg: &[u8]) -> G1Projective {
    <G1Projective as HashToCurve<ExpandMsgXmd<sha2::Sha256>>>::hash_to_curve(msg, CSUITE)
}

#[cfg(feature = "blst")]
pub fn hash(msg: &[u8]) -> G1Projective {
    G1Projective::hash_to_curve(msg, CSUITE, &[])
}
...
pub fn aggregate(signatures: &[Signature]) -> Result<Signature, Error> {
    ...
    let res = signatures
        .into_par_iter()
        .fold(G1Projective::identity, |mut acc, signature| {
            acc += &signature.0;
            acc
        })
...
pub fn aggregate(signatures: &[Signature]) -> Result<Signature, Error> {
    ...
    let res = signatures
        .into_iter()
        .fold(G1Projective::identity(), |acc, signature| {
            acc + &signature.0
        });
...
pub fn verify(signature: &Signature, hashes: &[G1Projective], public_keys: &[PublicKey]) -> bool {
...
pub fn verify_messages(
    ...
    let mut pairings = messages
        ...
        .map(|chunk| {
            let mut pairing = blstrs::PairingG2G1::new(true, CSUITE);
    ...
    if valid.load(Ordering::Relaxed) {
        blstrs::PairingG2G1::aggregated(&mut gtsig, &signature.0);
    }
